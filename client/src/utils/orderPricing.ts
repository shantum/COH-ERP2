 ...
 * 4. Return 0 with warning for normal orders (never silently fail)
 */

import type { Order, OrderLine } from '../types';

/**
 * Extended ShopifyOrderCache type that includes the generated totalPrice column
 * from the database. This field is computed from Shopify's total_price during sync.
 */
interface ExtendedShopifyCache {
  ...
  /** Generated column from Shopify's total_price - may not be present in base type */
  totalPrice?: number;
}
 ...
 */
function calculateTotalFromLines(orderLines: OrderLine[] | undefined): number {
 ...
 */
export function calculateOrderTotal(order: Order): OrderTotalResult {
  const isExchange = order.isExchange === true;
  const orderLines = order.orderLines;
  const shopifyCache = order.shopifyCache as ExtendedShopifyCache | null | undefined;
  ...
  // 4. No valid total found - return 0 with warning
  // This should only happen for orders with no lines or all cancelled lines
  if (import.meta.env?.DEV) {
    console.warn(
      `[orderPricing] Order ${order.orderNumber} has no valid total. ` +
      `totalAmount=${order.totalAmount}, shopifyCache.totalPrice=${shopifyCache?.totalPrice}, ` +
 ...
 * });
 */
export function getProductMrpForShipping(order: Order): number {
  ...
  if (import.meta.env?.DEV) {
    console.warn(
      `[orderPricing] Using minimum MRP fallback (${MINIMUM_MRP}) for order ${order.orderNumber}. ` +
      `This may indicate missing order data.`
    );
  }
 ...
 */
export function hasValidPricing(order: Order): boolean {
  const result = calculateOrderTotal(order);
