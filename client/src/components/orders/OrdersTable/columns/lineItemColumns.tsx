...
export function buildLineItemColumns(ctx: OrdersTableContext): ColumnDef<FlattenedOrderRow>[] {
    ...
    return [
        ...
        {
            id: 'mrp',
            header: getHeaderName('mrp'),
            size: DEFAULT_COLUMN_WIDTHS.mrp,
            cell: ({ row }) => <MrpCell row={row.original} />,
            enableSorting: true,
            sortingFn: (a, b) => (a.original.mrp || 0) - (b.original.mrp || 0),
        },
        {
            id: 'discount',
            header: getHeaderName('discount'),
            size: DEFAULT_COLUMN_WIDTHS.discount,
            cell: ({ row }) => <DiscountCell row={row.original} />,
            enableSorting: true,
            sortingFn: (a, b) => (a.original.discountPercent || 0) - (b.original.discountPercent || 0),
        },
        {
            id: 'cost',
