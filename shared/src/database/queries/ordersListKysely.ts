...
function applyViewWhereClause(
    _qb: ReturnType<typeof getKysely>['selectFrom'] extends (table: 'Order') => infer R ? R : never,
    view: ViewName,
    shippedFilter?: ShippedFilter
) {
    const kysely = getKysely();
    let baseQb = kysely.selectFrom('Order').select(sql<number>`COUNT(*)::int`.as('count'));

    switch (view) {
        case 'open':
            return baseQb.where((eb) =>
                eb.and([
...
export async function listOrdersKysely(params: OrdersListParams) {
    ...
    let countQuery = applyViewWhereClause(
        kysely.selectFrom('Order').select(sql<number>`COUNT(*)::int`.as('count')) as never,
        view,
        shippedFilter
    );
