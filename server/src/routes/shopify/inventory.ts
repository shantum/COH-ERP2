 ...
 */
router.get('/item/:sku', authenticateToken, asyncHandler(async (req: Request, res: Response): Promise<void> => {
    if (!shopifyClient.isConfigured()) {
        res.status(400).json({ error: 'Shopify is not configured' });
        return;
    }
 ...
 */
router.post('/set', requireAdmin, asyncHandler(async (req: Request, res: Response): Promise<void> => {
    if (!shopifyClient.isConfigured()) {
        res.status(400).json({ error: 'Shopify is not configured' });
        return;
    }
 ...
 */
router.post('/zero-out', requireAdmin, asyncHandler(async (req: Request, res: Response): Promise<void> => {
    if (!shopifyClient.isConfigured()) {
        res.status(400).json({ error: 'Shopify is not configured' });
        return;
    }
