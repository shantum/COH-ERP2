...
router.get('/pending', async (req, res) => {
    try {
        ...
        console.error('Get pending remittance error:', error);
        res.status(500).json({ error: error.message });
    }
});
...
router.get('/summary', async (req, res) => {
    try {
        ...
        console.error('Remittance summary error:', error);
        res.status(500).json({ error: error.message });
    }
});
...
router.get('/failed', async (req, res) => {
    try {
        ...
        console.error('Get failed sync error:', error);
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/retry-sync', async (req, res) => {
    try {
        ...
        if (all) {
            ...
            return res.status(400).json({ error: 'Provide orderIds array or set all=true' });
        }
...
router.post('/approve-manual', async (req, res) => {
    try {
        ...
        if (order.codShopifySyncStatus !== 'manual_review') {
            return res.status(400).json({ error: 'Order is not flagged for manual review' });
        }
        ...
        console.error('Approve manual sync error:', error);
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/reset', async (req, res) => {
    try {
        ...
        console.error('Remittance reset error:', error);
        res.status(500).json({ error: error.message });
    }
});
 ...
 */
router.post('/fix-payment-method', async (req, res) => {
    try {
