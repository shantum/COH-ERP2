...
/**
 * Find an existing unmatched inward transaction for a fabric colour.
 * Looks for recent inward txns that don't already have an invoice line linked.
 */
async function findUnmatchedInwardTxn(
    fabricColourId: string,
    qty: number | null,
    prisma: PrismaClient,
): Promise<string | null> {
    // Find recent inward transactions for this fabric colour that aren't already linked to an invoice line
    const candidates = await prisma.fabricColourTransaction.findMany({
        where: {
            fabricColourId,
            txnType: 'inward',
            invoiceLine: null, // Not yet linked
            createdAt: {
    ...
    // If we have quantity info, pick the closest match
    if (qty != null && qty > 0) {
        let bestCandidate = candidates[0];
        let bestDiff = Math.abs(candidates[0].qty - qty);

        for (const c of candidates) {
            const diff = Math.abs(c.qty - qty);
            if (diff < bestDiff) {
                bestDiff = diff;
                bestCandidate = c;
            }
        ...
        // Only match if quantity is within 20% tolerance
        const tolerance = qty * 0.2;
        if (bestDiff <= tolerance) {
            return bestCandidate.id;
        }
    }

    // No quantity to compare — return most recent unmatched
